...
SMOKE_DIR="$(mktemp -d)"
cleanup() {
	rm -f -- "$SMOKE_DIR"/*.json "$SMOKE_DIR"/*.err
	rmdir "$SMOKE_DIR"
}
trap cleanup EXIT
...
run_json() {
	local name="$1"
	shift
	local out="$SMOKE_DIR/${name//[^A-Za-z0-9_.-]/_}.json"
	local err="$SMOKE_DIR/${name//[^A-Za-z0-9_.-]/_}.err"
	printf '==> %s\n' "$name" >&2
...
run_optional_json() {
	...
	local out="$SMOKE_DIR/${name//[^A-Za-z0-9_.-]/_}.json"
	local status
	if ! status="$(json_value "$out" "http_status")"; then
		return 1
	fi
	case "$status" in
	403 | 404)
		printf 'SKIP %s: API returned HTTP %s for this account\n' "$name" "$status" >&2
		;;
	*) return 1 ;;
	esac
}
...
run_json "export-all" "${FRAME_FLAGS[@]}" export --resources all --days 1 --output-file "$SMOKE_DIR/export.json" --json >/dev/null
